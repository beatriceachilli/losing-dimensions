"""
Common layers for defining the noise conditional network
"""
...
def get_normalization(name, conditional=False):
...
def get_timestep_embedding(timesteps, embedding_dim, max_positions=10000):
    """From Fairseq: https://github.com/facebookresearch/fairseq
    ...
    pos = time_step
    Args:
        timesteps: a vector of same dimension as the batch. So one t per sample x -> x(t)
    Returns:
        A embedding_dim x embedding_dim matrix
    """
    assert len(timesteps.shape) == 1  # and timesteps.dtype == tf.int32
    half_dim = embedding_dim // 2  # for sin and cos vectors
